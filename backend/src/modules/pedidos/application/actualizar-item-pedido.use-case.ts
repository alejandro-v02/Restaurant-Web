...
export interface ActualizarItemPedidoInput {
  cantidad?: number;
  notas?: string;
}

@Injectable()
export class ActualizarItemPedidoUseCase {
  constructor(
  ...
  async execute(
    ...
    input: ActualizarItemPedidoInput,
  ): Promise<PedidoItem> {
    ...
    if (item.estado !== EstadoPedidoItem.PENDIENTE) {
      throw new ConflictException('Ya se está preparando, no se puede modificar');
    }
